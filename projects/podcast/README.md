...
贴纸、大卡和标签效果配置在 `effects_plan.json`。剧情段会在单词卡位置显示明显的梗卡，生词段再切换成单词卡；如果规则里配置了 `emojis`，渲染器会从 `assets/animated_emojis` 读取动态 GIF 做贴纸。详细说明见 `video_generation_guide.md`。
