...
接下来第七章进入当前主流 TTS 模型方案，第八章讲 acoustic model（声学模型），第九章讲波形还原模块（vocoder、codec decoder、AudioVAE decoder），第十章再专门展开 diffusion（扩散模型）和 flow matching（流匹配）如何在 mel、waveform、latent 或 codec token 空间中做生成。
