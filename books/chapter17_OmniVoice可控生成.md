# 第十七章：OmniVoice 可控生成 —— 音色克隆、声音设计与推理参数

完成第十六章的 zero-shot voice cloning（零样本声音克隆）后，本章继续看 OmniVoice 的可控生成能力。
...
## 17.1 voice cloning：用 `ref_audio` 控制音色
...
## 17.2 voice design：用 `instruct` 描述属性
...
## 17.4 `ref_audio` 和 `instruct` 不要先假设完全解耦
...
## 17.5 推理参数：`num_step`、`guidance_scale`、`speed`、`duration`
